...
- 🚨 ถ้าเจอ "Silver Dollar Eucalyptus" หรือ "ยูคาใบเงิน" ให้ใช้ชื่อ "สนเกรวิลเลีย" แทน (ยูคาใบเงินในไทยไม่นิยม คนมักมองผิด)
- 🚨 ถ้าเจอต้นไม้ที่คล้าย "สนฉัตร" (ต้นไม้ทรงพุ่มใบเขียวเข้ม ใบเล็กเรียงเป็นชั้น) หรือไม่แน่ใจระหว่าง "สนฉัตร" กับ "ไทรใบสัก" ให้ใช้ชื่อ "สนฉัตร" แทน (AI มักมองผิดเป็นไทรใบสัก)`;
    ...
    // 4. แทนที่ชื่อต้นไม้ที่ไม่มีจริงในไทยด้วยชื่อที่ถูกต้อง
    const correctedPlants = finalPlants.map(plant => {
      const name = plant.name || '';
      const normalizedName = name.toLowerCase();
      
      // แทนที่ "ยูคาใบเงิน" หรือ "Silver Dollar Eucalyptus" ด้วย "สนเกรวิลเลีย" (ยูคาใบเงินในไทยไม่นิยม คนมักมองผิด)
      if (normalizedName.includes('ยูคาใบเงิน') || 
          normalizedName.includes('silver dollar eucalyptus') || 
          normalizedName.includes('silver dollar') && normalizedName.includes('eucalyptus')) {
        console.log(`🔄 แทนที่ "${name}" ด้วย "สนเกรวิลเลีย" (ยูคาใบเงินในไทยไม่นิยม)`);
        return {
          ...plant,
          name: 'สนเกรวิลเลีย',
          fallbackUsed: true,
      ...
      // แทนที่ "ไทรใบสัก" ที่มีลักษณะคล้าย "สนฉัตร" ด้วย "สนฉัตร" (AI มักมองผิด)
      // ตรวจสอบจาก description หรือ notes ที่มีคำว่า "พุ่ม", "ใบเล็ก", "เรียงเป็นชั้น", "ทรงพุ่ม"
      if (normalizedName.includes('ไทรใบสัก') || normalizedName.includes('ficus lyrata')) {
        const description = (plant.description || '').toLowerCase();
        const notes = (plant.notes || '').toLowerCase();
        const hasPineLikeFeatures = description.includes('พุ่ม') || 
                                   description.includes('ใบเล็ก') || 
                                   description.includes('เรียงเป็นชั้น') ||
                                   description.includes('ทรงพุ่ม') ||
                                   notes.includes('พุ่ม') ||
                                   notes.includes('ใบเล็ก') ||
                                   notes.includes('เรียงเป็นชั้น');
        
        if (hasPineLikeFeatures) {
          console.log(`🔄 แทนที่ "${name}" ด้วย "สนฉัตร" (AI มักมองผิดเป็นไทรใบสัก)`);
          return {
            ...plant,
            name: 'สนฉัตร',
            fallbackUsed: true,
      ...
      // แทนที่ "สนคอลัมน์" หรือ "Columnar Pine" ด้วย "สนมังกร" (นิยมในไทยมากที่สุด)
      if (normalizedName.includes('สนคอลัมน์') || 
          normalizedName.includes('columnar pine') || 
          normalizedName.includes('columnar') && normalizedName.includes('pine')) {
        console.log(`🔄 แทนที่ "${name}" ด้วย "สนมังกร" (ต้นไม้ที่นิยมในไทย)`);
        return {
          ...plant,
          name: 'สนมังกร',
          fallbackUsed: true,
    ...
    const sortedResult = {
      ...analysisResult,
      plants: correctedPlants, // ต้นไม้มาก่อน (อยู่แล้วใน plants array)
