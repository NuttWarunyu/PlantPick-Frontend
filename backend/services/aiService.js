...
⚠️ **คำแนะนำสำคัญ (บังคับต้องทำตาม):**

1. **วิเคราะห์ต้นไม้ทุกต้นที่เห็นในภาพ** (ไม่ใช่แค่ 4-5 ต้น)
   - ต้องวิเคราะห์ต้นไม้ทุกต้นที่เห็น รวมถึงต้นเล็ก ต้นใหญ่ ไม้พุ่ม ไม้คลุมดิน
   - ต้องมีต้นไม้อย่างน้อย 6-10 ชนิดขึ้นไป (ถ้าเห็นในภาพ)
   - วิเคราะห์ตามลำดับ: Focal Tree → Columnar → Round Shrubs → Silver/Unique → Low Shrubs → Flowering

2. **บังคับ: field "name" ต้องระบุชื่อต้นไม้ที่เฉพาะเจาะจงเท่านั้น** (ไม่ใช่คำอธิบาย)
   - ห้ามใช้คำอธิบายทั่วไป เช่น "พุ่มไม้", "หญ้าประดับ", "ไม้พุ่มดอกสีฟ้า", "ต้นสูง ใบเขียว", "พุ่มกลม ใบเขียว"
   - ต้องระบุชื่อต้นไม้จริงๆ เช่น:
     - Focal Tree: "มะกอกฝรั่ง", "ยูคาใบเงิน", "ไทรใบสัก", "ยูจีเนีย"
     - Columnar: "สนคอลัมน์", "สนสก็อต", "จูนิเปอร์", "ปาล์ม"
     - Round Shrubs: "ชาฮกเกี้ยนตัดบอล", "บอกซ์วูด", "ไทรใบกลม", "โกสน", "เข็ม"
     - Silver/Unique: "บลูยัคคา", "ดาซิลิเรียน", "อากาเว่"
     - Low Shrubs: "หญ้าน้ำพุแคระ", "ลิริโอเป", "กวักมรกต", "เฟิร์น", "พลูด่าง", "เดหลี"
     - Flowering: "ลาเวนเดอร์", "บลูพลัมบาโก", "ดาวเรือง", "บานชื่น", "พุทธรักษา"

3. **บังคับ: field "category" ต้องระบุหมวดหมู่ต้นไม้** (focal_tree, columnar, round_shrub, silver_unique, low_shrub_groundcover, flowering)

4. **วิเคราะห์หินตกแต่งและองค์ประกอบอื่นๆ**
   - ถ้าเห็นหินตกแต่ง ต้องระบุใน otherElements (เช่น หินก้อนสีขาว Limestone/Marble Rock)
   - ระบุจำนวน, ขนาด, และตำแหน่ง

5. **รายละเอียดที่ต้องมีครบถ้วน**
   - จำนวนต้น (ตัวเลข เช่น 1, 2, 5, 10, 15)
   - ขนาด (เช่น สูง 2.5-3 เมตร, Ø 40-60 ซม.)
   - ตำแหน่ง (เช่น ซ้ายบน, กลาง, ขวาล่าง)
   - ลักษณะ (เช่น ต้นกลางพุ่มโปร่ง ใบสีเงิน, พุ่มกลมเขียวเข้ม)

6. **ถ้าไม่แน่ใจ ให้เดาชื่อที่ใกล้เคียงที่สุด** (ผิดก็ไม่เป็นไร - แต่ต้องมีชื่อมาเพื่อนำไปเสนอราคา)

7. **ถ้าไม่มีองค์ประกอบใดๆ ให้ใส่ null หรือ [] ตามประเภท**
