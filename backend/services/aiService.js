...
1. **ต้นไม้**: ระบุชื่อต้นไม้ที่ถูกต้องและเฉพาะเจาะจงที่สุด พร้อมจำนวน ขนาด และตำแหน่ง
   - ระบุชื่อต้นไม้ที่ถูกต้องและเฉพาะเจาะจง (เช่น "ชาฮกเกี้ยน", "ยูจีเนีย", "ไทรใบสัก", "มอนสเตอร่า", "เศรษฐีเรือนใน")
   - ระบุจำนวนต้น (ตัวเลข เช่น 5, 10, 15)
   - ระบุขนาด (เช่น 0.5-1 เมตร, 1-2 ฟุต, 3-4 เมตร)
   - ระบุตำแหน่งในภาพ (เช่น ซ้ายบน, กลาง, ขวาล่าง)
   - ระบุลักษณะ (เช่น พุ่มกลม, ต้นสูง, ไม้เลื้อย, ใบเขียว, ดอกสีแดง)
   - ⚠️ สำคัญ: พยายามระบุชื่อต้นไม้ที่ถูกต้องและเฉพาะเจาะจงที่สุดเท่าที่ทำได้ (ไม่ใช่แค่คำอธิบายทั่วไป เช่น "พุ่มกลม ใบเขียว")
...
กรุณาตอบเป็น JSON format ตามโครงสร้างนี้:
{
  "plants": [
    {
      "name": "ชื่อต้นไม้ที่ถูกต้องและเฉพาะเจาะจง (เช่น ชาฮกเกี้ยน, ยูจีเนีย, ไทรใบสัก, มอนสเตอร่า) หรือ null ถ้าไม่แน่ใจ",
...
- **สำคัญมาก**: พยายามระบุชื่อต้นไม้ที่ถูกต้องและเฉพาะเจาะจงที่สุด (เช่น "ชาฮกเกี้ยน", "ยูจีเนีย", "ไทรใบสัก", "มอนสเตอร่า", "เศรษฐีเรือนใน")
- หลีกเลี่ยงคำอธิบายทั่วไป (เช่น "พุ่มกลม ใบเขียว", "ต้นสูง ใบเขียว") - ให้ระบุชื่อต้นไม้ที่แท้จริง
- ถ้าไม่แน่ใจชื่อต้นไม้ ให้ใส่ null ใน field "name" แต่ให้ description ที่ละเอียด
