...
1. **ต้นไม้**: ระบุชื่อต้นไม้ที่เฉพาะเจาะจง (บังคับต้องมีชื่อ - ไม่ใช่คำอธิบาย)
   - ⚠️ สำคัญมาก: ต้องระบุชื่อต้นไม้ที่เฉพาะเจาะจงเท่านั้น (ไม่ใช่คำอธิบาย)
   - เน้นต้นไม้ที่นิยมจัดสวนในไทย: ชาฮกเกี้ยน, ยูจีเนีย, ไทรใบสัก, ไทรใบกลม, ไทรใบเล็ก, มอนสเตอร่า, เศรษฐีเรือนใน, พลูด่าง, เดหลี, ว่านหางจระเข้, กวักมรกต, เฟิร์น, ปาล์ม, หมากผู้หมากเมีย, โกสน, เข็ม, หน้าวัว, ดาวเรือง, บานชื่น, ดาวกระจาย, พุทธรักษา, เบญจมาศ, แก้ว, มะยม, ชมพู่, มะม่วง, กล้วย, มะนาว, ตะไคร้, ขิง, ข่า, พริก, มะเขือ, ผักชี, โหระพา, กะเพรา
   - ถ้าไม่แน่ใจ ให้เดาชื่อที่ใกล้เคียงที่สุด (ผิดก็ไม่เป็นไร - แต่ต้องมีชื่อมา)
   - ห้ามใช้คำอธิบายทั่วไป เช่น "พุ่มไม้", "หญ้าประดับ", "ไม้พุ่มดอกสีฟ้า" - ต้องระบุชื่อต้นไม้จริงๆ
   - ระบุจำนวนต้น (ตัวเลข เช่น 5, 10, 15)
   - ระบุขนาด (เช่น 0.5-1 เมตร, 1-2 ฟุต, 3-4 เมตร)
   - ระบุตำแหน่งในภาพ (เช่น ซ้ายบน, กลาง, ขวาล่าง)
...
กรุณาตอบเป็น JSON format ตามโครงสร้างนี้:
{
  "plants": [
    {
      "name": "ชื่อต้นไม้ที่เฉพาะเจาะจง (บังคับต้องมี - ไม่ใช่คำอธิบาย เช่น ชาฮกเกี้ยน, ยูจีเนีย, ไทรใบสัก, มอนสเตอร่า, เศรษฐีเรือนใน, พลูด่าง, เดหลี, กวักมรกต, เฟิร์น, ปาล์ม, โกสน, เข็ม)",
      "quantity": จำนวนต้น (ตัวเลข เช่น 5, 10, 15),
      "size": "ขนาด (เช่น 0.5-1 เมตร, 1-2 ฟุต, 3-4 เมตร)",
      ...
      "description": "ลักษณะต้นไม้ (เช่น พุ่มกลม, ต้นสูง, ไม้เลื้อย, ใบเขียว, ดอกสีแดง)",
...
- **สำคัญมาก - บังคับ**: field "name" ต้องระบุชื่อต้นไม้ที่เฉพาะเจาะจงเท่านั้น (ไม่ใช่คำอธิบาย)
- ห้ามใช้คำอธิบายทั่วไป เช่น "พุ่มไม้", "หญ้าประดับ", "ไม้พุ่มดอกสีฟ้า", "ต้นสูง ใบเขียว", "พุ่มกลม ใบเขียว"
- ต้องระบุชื่อต้นไม้จริงๆ เช่น "ชาฮกเกี้ยน", "ยูจีเนีย", "ไทรใบสัก", "มอนสเตอร่า", "เศรษฐีเรือนใน", "พลูด่าง", "เดหลี", "กวักมรกต", "เฟิร์น", "ปาล์ม", "โกสน", "เข็ม"
- ถ้าไม่แน่ใจ ให้เดาชื่อที่ใกล้เคียงที่สุด (ผิดก็ไม่เป็นไร - แต่ต้องมีชื่อมาเพื่อนำไปเสนอราคา)
- เน้นต้นไม้ที่นิยมจัดสวนในไทย
- จำนวนและขนาดให้เป็นตัวเลขประมาณ
- วิเคราะห์ต้นไม้ทีละต้นอย่างละเอียด
