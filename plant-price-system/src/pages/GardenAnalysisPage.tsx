...
const GardenAnalysisPage: React.FC = () => {
  ...
  const handleFileSelect = (file: File) => {
    ...
    if (!supportedTypes.includes(file.type.toLowerCase()) && !supportedExtensions.test(file.name)) {
      setError('รูปแบบไฟล์ไม่รองรับ กรุณาใช้ JPEG, PNG, GIF, WebP หรือ HEIF/HEIC (จะถูกแปลงอัตโนมัติ)');
      return;
    }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // แสดง error message ที่เข้าใจง่ายสำหรับ mobile
      let errorMessage = err.message || 'เกิดข้อผิดพลาดในการวิเคราะห์รูปภาพ';
      
      // ปรับ error message ให้เข้าใจง่ายขึ้น
      if (errorMessage.includes('timeout')) {
        errorMessage = '⏱️ การวิเคราะห์ใช้เวลานานเกินไป\n\n💡 คำแนะนำ:\n- ลองเลือกรูปภาพที่มีขนาดเล็กลง\n- ตรวจสอบสัญญาณอินเทอร์เน็ต\n- ลองใหม่อีกครั้ง';
